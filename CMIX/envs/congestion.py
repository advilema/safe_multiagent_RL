...
class Congestion(object):
    ...
    def __init__(self, size, n_agents, avg_cost=1, tau=0.999, noise=0.1, shuffle=False):
        self.size = size
        self.n_agents = n_agents
        self.agents = [Agent(i, size) for i in range(n_agents)]
        self.landmark = [size, size]  # upper left square
        self.action_space = [5] * self.n_agents  # up, down, left, right, stay
    ...
    def step(self, action):
        ...
        local_reward = [[rew - self.delta, self.avg_cost_ubound - sum(constraint) / len(constraint)] for rew in reward]
        global_reward = local_reward[0]

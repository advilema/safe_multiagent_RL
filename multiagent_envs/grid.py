...
class Grid(object):
    ...
    def __init__(self, size, n_agents, n_landmarks, shuffle=True, agents_size = 0.5):
        ...
        self.start_landmarks = [np.floor(np.random.rand(2)*size).tolist() for _ in range(n_landmarks)]
        self.landmarks = self.start_landmarks.copy()
        self.shuffle = shuffle
        self.action_space = 5 #up, down, left, right, stay
        self.state_space = 2*n_agents
        if self.shuffle:
            self.state_space += 2*self.n_landmarks #add 2D positions of the landmarks
        self.viewer = None
    ...
    def _reset_landmarks(self):
        self.landmarks = [np.floor(np.random.rand(2) * self.size).tolist() for _ in range(self.n_landmarks)]
    ...
    def check_done(self):
        ...
        for agent in self.agents:
            ...
            for landmark in self.landmarks:
                if np.all(np.array(agent.state) - np.array(landmark) == 0):
                    agent.done = True
                    done.append(True)
            if not agent.done:
                done.append(False)
...
if __name__ == '__main__':
    ...
    env = Grid(size, n_agents, n_landmarks, shuffle=False)
